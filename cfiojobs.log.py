...
#peak_value_sheet('max')
    ...
    sheet_title = u'主机名,测试设备,块大小/模式,该测试带宽(MiB/s),测试带宽平均值(MiB/s),相对平均带宽的比值,同组筛选状态,该测试每秒读写(iops),测试每秒读写(iops)平均值,该测试平均延迟(ms),测试延迟平均值(ms),该测试最大延迟(ms),该测试最低延迟(ms),读写队列深度,该测试设备使用率,测试数据量,测试时长,读写数据引擎\n'.encode('GB2312')
