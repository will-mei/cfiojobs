...
4. 测试的包含一定参数范围(如4k,256k,4m多个块大小)的作业为一个作业组,
        配置文件:
        cfiojobs.job
...
10. 通用测试步骤
    ...
    1). 向配置文件添加主机组信息
        将测试主机最基本信息,组名等填写在'cfiojobs.grp'/'conf/<组名>.grp'中,(建议清除示例的主机模版和配置文件),更加易读.
        若主机数相对较少,
            可以将主机ip列表等信息填到'cfiojobs.grp'中,简单快速
        或者,当主机数量较多时,
            可以使用独立文件配置主机组信息'conf/<组名>.grp',每行一个独立的IP地址,可以使用'#'注释废弃的主机
            注意,'conf/<组名>.grp'目录存储的是主配置文件'cfiojobs.grp'的扩展,
            因此,该组的登录用户与端口仍然需要债主配置文件中指定.
    ...
    4). 确定测试作业
        填写或者选取'cfiojobs.job'中的作业组,直接运行 'group.sh' 即可查看默认的作业信息
        如需调整,请手动更改'group.sh'脚本中的默认作业组变量 'job_group=<需要使用的作业组名>'

    5). 运行测试脚本,等待测试结束
        直接运行 'group.sh' 即可查看自动测试的默认参数,确认无误后请运行'group.sh' 并指定模式和报告的Excel名称即可开始测试.
        使用 './cfiojobs -l' 可以快速查看正在运行测试的主机状态,
        更详细的进度,可以查看'group.sh'同目录下的对应测试组测试日志,文件名'<组名><模式><日期>.log'
